...
import { flattenZodErrors, formatError, escapeRegExp } from "@/lib/utils";
...
export async function createWebPage(
  ...
  try {
    const validated = WebPageInputSchema.safeParse(data);
...
export async function updateWebPage(
  ...
  try {
    const validated = WebPageUpdateSchema.safeParse(data);
    if (!validated.success) {
      return {
        success: false,
        message: "Validation failed",
...
export async function getAllWebPages({
  ...
  const filter: FilterQuery<IWebPage> = {};
  if (query) {
