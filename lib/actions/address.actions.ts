...
import { flattenZodErrors, formatError } from "@/lib/utils";
...
): Promise<ActionState<AddressBookEntry[]>> {
  try {
    const { session, addresses } = await getCurrentUserAddresses();
    if (!session?.user?.id) throw new Error("You must be signed in");

    const validated = AddressBookInputSchema.safeParse(payload);
