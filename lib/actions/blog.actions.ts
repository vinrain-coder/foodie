...
export async function createBlog(
  ...
  try {
    const validated = BlogInputSchema.safeParse(data);
...
export async function toggleBlogLike(
   input: z.infer<typeof BlogLikeInputSchema>
): Promise<ActionState<{ liked: boolean; likesCount: number }>> {
  try {
    const validated = BlogLikeInputSchema.safeParse(input);
    if (!validated.success) {
      return {
        success: false,
        message: "Validation failed",
    ...
    return { success: true, data: { liked: !hasLiked, likesCount: blog.likesCount } };
  } catch (error) {
...
export async function createBlogComment(
  input: z.infer<typeof BlogCommentInputSchema>
): Promise<ActionState> {
  try {
    const session = await getServerSessionLazy();
    if (!session) throw new Error("Please sign in to comment");

    const validated = BlogCommentInputSchema.safeParse(input);
    if (!validated.success) {
      return {
        success: false,
        message: "Validation failed",
...
export async function toggleBlogCommentLike(
  ...
  try {
    ...
    return { success: true, data: { liked: !hasLiked, likesCount: target.likesCount } };
  } catch (error) {
    return { success: false, message: formatError(error) };
  }
}
