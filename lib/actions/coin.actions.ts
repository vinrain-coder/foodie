...
import { AdminCoinAdjustmentInputSchema } from "../validator";
...
export async function getUserCoinHistoryAdmin({
  ...
  const history = rawEvents.map((eventRaw: unknown) => {
    const event = eventRaw as Record<string, any>;
...
export async function adjustUserCoinsAdmin({
  ...
  try {
    ...
    const validated = AdminCoinAdjustmentInputSchema.safeParse({
      userId,
      amount,
      reason,
    });
