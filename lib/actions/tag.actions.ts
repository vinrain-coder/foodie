...
import { formatError, escapeRegExp, flattenZodErrors } from "@/lib/utils";
...
export async function createTag(
  data: z.infer<typeof TagInputSchema>
): Promise<ActionState> {
  try {
    const validated = TagInputSchema.safeParse(data);
...
export async function updateTagAction(
  ...
  try {
    const validated = TagUpdateSchema.safeParse(data);
    if (!validated.success) {
      return {
        success: false,
        message: "Validation failed",
