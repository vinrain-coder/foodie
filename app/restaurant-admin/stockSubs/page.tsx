...
export const metadata: Metadata = {
  title: "Restaurant Stock Subscriptions",
};
...
export default async function StockSubscriptionsPage(props: {
  ...
  // Determine if we are filtering by a specific menuItem to show "Notify All"
  // If query is present and only returns one unique menuItem, or if we have results.
  const uniqueMenuItemIds = [
    ...new Set(
      data.data
        .filter((sub: any) => !sub.isNotified && sub.menuItem)
        .map((sub: any) => sub.menuItem._id.toString()),
    ),
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-md border bg-card p-4 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <StockSubFilters />
        {uniqueMenuItemIds.length === 1 && (
          <NotifyButton
            menuItemId={uniqueMenuItemIds[0] as string}
