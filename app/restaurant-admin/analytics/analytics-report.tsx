...
import { CalendarDateRangePicker } from "@/app/restaurant-admin/overview/date-range-picker";
import {
  Card,
  CardContent,
  CardDescription,
...
export default function AnalyticsReport() {
  ...
  const [data, setData] =
    useState<Awaited<ReturnType<typeof getWebAnalyticsSummary>>>();
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
          {[...Array(4)].map((_, index) => (
            <Skeleton key={index} className="h-82.5 rounded-xl" />
          ))}
        </div>
      </div>
    );
    ...
    <div className="space-y-6 pb-10">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Understand traffic quality, acquisition, and engagement across your
            storefront.
          </p>
        </div>
              ...
              <AreaChart data={data.pageViewsByDay}>
                ...
                <Tooltip
                  labelFormatter={(label) =>
                    formatDateTime(new Date(label)).dateOnly
                  }
                />
                <Area
                  type="monotone"
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        ...
        <TopTableCard
          ...
          rows={data.topBrowsers.map(
            (item: { name: string; views: number }) => ({
              browser: item.name,
              views: item.views,
            }),
          )}
        ...
        <Card className="xl:col-span-2 border-border/70 shadow-none">
          <CardHeader>
            ...
            <CardDescription>
              Distribution of traffic by device class.
            </CardDescription>
          </CardHeader>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  >
                    {data.topDevices.map(
                      (
                        entry: { name: string; views: number },
                        index: number,
                      ) => (
                        <Cell
                          key={entry.name}
...
function MetricCard({
  ...
  return (
    <Card
      className={`relative overflow-hidden border-l-4 shadow-sm transition-all hover:shadow-md ${toneClasses.border}`}
    >
