              ...
              <TableRow key={sub._id}>
                  ...
                  {sub.menuItem ? (
                    <div className="flex items-center gap-3">
                      <div className="relative h-10 w-10 overflow-hidden rounded border">
                        <Image
                          src={sub.menuItem.images[0]}
                          alt={sub.menuItem.name}
                          fill
                      ...
                      <Link
                        href={`/admin/menu-items/${sub.menuItem._id}`}
                        className="font-medium hover:underline line-clamp-1 max-w-64"
                      >
                        {sub.menuItem.name}
                      </Link>
                      {!sub.menuItem.isPublished && (
                        <Badge
                          variant="outline"
                ...
                <TableCell className="text-right">
                  <div className="flex justify-end gap-2">
                    {!sub.isNotified && sub.menuItem && (
                      <NotifyButton
                        subscriptionId={sub._id}
                        menuItemId={sub.menuItem._id}
                      />
                    )}
                    <DeleteDialog
                      id={sub._id}
