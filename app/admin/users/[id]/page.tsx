...
import { ArrowLeft, Clock3, UserCircle } from "lucide-react";
...
export default async function UserEditPage(props: {
  ...
  const { user, metrics, monthlyOrders, recentOrders, navigationHistory } =
    data;
    ...
    <main className="mx-auto w-full max-w-7xl space-y-4 px-0 sm:px-2">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Full customer profile, order insights, wishlist snapshot, and
            navigation activity.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
        <Card className="xl:col-span-2">
          <CardHeader>
            ...
            <CardDescription>
              Monthly order volume for this user.
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          ...
          <CardContent className="space-y-3 text-sm">
            ...
            <div className="flex items-center justify-between">
              ...
              <span>
                <Price price={metrics.avgOrderValue} plain />
              </span>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Latest saved menuItems.</CardDescription>
          </CardHeader>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="relative h-10 w-10 overflow-hidden rounded border">
                        <Image
                          src={
                            menuItem.images?.[0] || "/images/placeholder.jpg"
                          }
                          alt={menuItem.name}
                          fill
                    ...
                    <Button asChild size="sm" variant="outline">
                      <Link href={`/menu-item/${menuItem.slug}`}>Open</Link>
                    </Button>
                  </div>
                ),
              )
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Most recent pages visited (tracked for signed-in users from this
            deployment onward).
          </CardDescription>
        </CardHeader>
        <CardContent>
          {navigationHistory.length > 0 ? (
            <div className="space-y-2">
              {navigationHistory.map(
                (
                  item: { path: string; title?: string; visitedAt: Date },
                  index: number,
                ) => (
                  <div
                    key={`${item.path}-${index}`}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Update profile and role with immediate admin control.
          </CardDescription>
        </CardHeader>
        <CardContent>
