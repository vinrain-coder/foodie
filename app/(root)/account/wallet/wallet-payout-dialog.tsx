...
import { WalletPayoutInputSchema } from "@/lib/validator";
...
export function WalletPayoutDialog({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const validated = WalletPayoutInputSchema.safeParse({
      amount,
      paymentMethod,
    ...
    if (!validated.success) {
      toast.error(
        validated.error.issues[0]?.message || "Please enter valid payout details",
      );
      return;
    }
    ...
    try {
      const res = await createWalletPayoutRequest({
        ...
        paymentDetails: validated.data.paymentDetails,
      });
