...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: toAbsoluteUrl(site.url, `/page/${slug}`),
    },
    openGraph: {
      title: webPage.title,
      description: description,
      url: toAbsoluteUrl(site.url, `/page/${slug}`),
      type: "article",
      publishedTime: new Date(webPage.createdAt).toISOString(),
      images: [
        {
          url: webPage.image || toAbsoluteUrl(site.url, "/icons/logo.png"),
          width: 1200,
          height: 630,
          alt: webPage.title,
        },
    ...
    twitter: {
      ...
      title: webPage.title,
      description: description,
      images: [webPage.image || toAbsoluteUrl(site.url, "/icons/logo.png")],
    },
  };
}
...
export default async function WebPage({ params }: PageProps) {
  ...
  const jsonLd = {
    ...
    image: webPage.image || toAbsoluteUrl(site.url, "/icons/logo.png"),
    ...
    publisher: {
      ...
      logo: {
        "@type": "ImageObject",
        url: toAbsoluteUrl(site.url, site.logo || "/icons/logo.png"),
      },
    },
    mainEntityOfPage: {
      "@type": "WebPage",
      "@id": toAbsoluteUrl(site.url, `/page/${slug}`),
    },
  };
