  ...
  persist<CartState>(
    (set, get) => ({
      ...
      addItem: async (item: OrderItem, quantity: number) => {
        const { items, shippingAddress, deliveryDateIndex, discount } =
          get().cart;
        const existItem = items.find((x) => x.menuItem === item.menuItem);
      ...
      updateItem: async (
        ...
        const duplicate = items.find(
          (x) => x.clientId !== item.clientId && x.menuItem === item.menuItem,
        );
        ...
        if (duplicate) {
          ...
          updatedCartItems = items.map((x) =>
            x.clientId === item.clientId ? { ...x, quantity } : x,
          );
        }
        const result = await calcDeliveryDateAndPrice({
          items: updatedCartItems,
