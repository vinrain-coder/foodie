...
function getGuestId() {
  ...
  const existing = window.localStorage.getItem("tumafood-blog-guest-id");
  if (existing) return existing;
  const guestId = crypto.randomUUID();
  window.localStorage.setItem("tumafood-blog-guest-id", guestId);
  return guestId;
}
...
export default function BlogSocial({
  ...
  const handlePostLike = async () => {
    ...
    setLikedPost(Boolean(response.data?.liked));
    setLikesCount(response.data?.likesCount ?? likesCount);
    ...
    if (!response.success) {
     ...
     setComments((prev) =>
       prev.map((c) => {
         if (c._id !== commentId) return c;
         if (!replyId)
           return { ...c, likesCount: response.data?.likesCount ?? c.likesCount };
         return {
           ...c,
           replies: c.replies.map((r) =>
             r._id === replyId
               ? { ...r, likesCount: response.data?.likesCount ?? r.likesCount }
               : r,
           ),
         };
       }),
