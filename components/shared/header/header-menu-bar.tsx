...
export default function HeaderMenuBar({ headerMenus }: { headerMenus: HeaderMenu[] }) {
  ...
  return (
    <div className="flex flex-1 items-center gap-1 overflow-x-auto whitespace-nowrap px-1 py-1 [scrollbar-width:thin]">
        ...
        if (!hasSubMenus) {
          return (
            <Link
              key={menu.href}
              href={menu.href}
              className="header-button shrink-0 rounded-md px-3 py-2 text-sm font-medium"
            >
              {menu.name}
            </Link>
          );
  ...
  return (
    <DropdownMenu open={open} onOpenChange={handleOpenChange}>
      <DropdownMenuTrigger asChild className="cursor-pointer">
        <button
          type="button"
          className="header-button inline-flex shrink-0 items-center gap-1 rounded-md px-3 py-2 text-sm font-medium outline-none"
      ...
      <DropdownMenuContent
        align="start"
        className="w-60 rounded-xl border-border/70 p-1.5 shadow-md"
        onMouseEnter={handleMouseEnter}
        onMouseLeave={handleMouseLeave}
      >
