...
export default function Sidebar({ categories }: SidebarProps) {
  ...
  return (
    <Drawer direction="left">
      {/* Trigger */}
      <DrawerTrigger className="header-button cursor-pointer rounded-md px-3 py-2">
        <MenuIcon className="h-5 w-5" />
        <span className="text-sm font-semibold">All</span>
      </DrawerTrigger>

      {/* Drawer Content */}
      <DrawerContent className="w-87.5 border-r border-border shadow-lg">
        <div className="flex h-full flex-col">
          ...
          <div className="flex flex-col gap-2 border-t p-4">
            <DrawerClose asChild>
              <Link href="/track" className="item-button rounded-md px-4 py-2">
                Track Order
              </Link>
            </DrawerClose>
